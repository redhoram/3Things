...
`Start from the End` is for **planning toward a goal** — a target you want to reach. If the user instead wants to *explain how something already works*, step by step, use a simple forward **Water Flow** (Input → Process → Output) rather than backward planning. Same three-part, time-based backbone; different viewpoint. Backward planning looks from the finish line and turns around; Water Flow looks forward step by step.
